...
test_set_size = 0.3
X_train_validation, X_test, y_train_validation, y_test = train_test_split(X, y, test_size=test_set_size,
                                                                          random_state=train_validation_random_seed)

# +------------------------------------------------------------
# | optimisation of k-NN
# | try different k-values with 10-fold cross-validation
...
for k in k_neighbours:
    ...
    scores = cross_val_score(knn, X_train_validation, y_train_validation, cv=kfold_splits, scoring='accuracy',
                             verbose=True)
    ...
    print("k_accuracy_score =", k_accuracy_score)
...
# kNN doesn't require training. However, the kNN algorithm in scikit-learn
# requires to implement a 'training', which actually load your train_validation data
knn = KNeighborsClassifier(n_neighbors=best_k)

knn.fit(X_train_validation, y_train_validation)  # implement the 'fitting' to load the combined train_validation
